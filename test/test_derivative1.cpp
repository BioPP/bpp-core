...
int main()
{
  ...
  for (unsigned int repeat = 0; repeat < 10000; ++repeat)
  {
    ...
    for (size_t i = 0; i < pl.size(); ++i)
    {
      ...
      if (abs(derivativesAna[i] - derivativesNum2pt[i]) > std::sqrt(nd2pt.getInterval()))
        test = false;
      if (abs(derivativesAna[i] - derivativesNum3pt[i]) > std::sqrt(nd3pt.getInterval()))
        test = false;
      if (abs(derivativesAna[i] - derivativesNum5pt[i]) > std::sqrt(nd5pt.getInterval()))
        test = false;
    }
